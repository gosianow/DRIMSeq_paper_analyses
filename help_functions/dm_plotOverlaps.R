...
calculateOverlaps <- function(results1, results2, by = 1){
  ...
  for(i in 1:length(top_ds_genes)){
    ...
    overlap[i] <- length(intersect(results1[1:top_ds_genes[i], "gene_id"], results2[1:top_ds_genes[i], "gene_id"]))
...
plotOverlaps <- function(data_Overlaps, metadata, plot_var, facet_var = NULL, plot_colors = NULL, plotx = TRUE, reference_color = NULL){
  
  stopifnot(length(data_Overlaps) == nrow(metadata))
  
  stopifnot(all(sapply(data_Overlaps, function(x){
  ...
  overlapslist <- lapply(1:length(data_Overlaps), function(r){
    # r = 1
    
    overlaps <- data_Overlaps[[r]]$overlaps
    
    overlaps <- cbind(overlaps, metadata[rep(r, nrow(overlaps)), , drop = FALSE])
    
  })
  
  Xlist <- lapply(1:length(data_Overlaps), function(r){
    # r = 1
