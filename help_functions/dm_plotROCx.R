...
#'  @name plotROCx
#'  @export
calculateROCx <- function(results, status, P = NULL, N = NULL){
  ...
  ROCXlist <- lapply(1:length(results), function(m){
    ...
    if(!all(c("gene_id", "pvalue") %in% colnames(results[[m]]))){
      
      message("Columns 'gene_id' or 'pvalue' are missing in results ", m, "\nReturn NAs")
      ...
      if("adj_pvalue" %in% colnames(results[[m]]) ){
        ...
        sts <- sts[apvs < 0.05]
        X <- data.frame(FPR = sum(!sts) / N, TPR = sum(sts) / P) 
...
#' @rdname plotROCx
#' @export
plotROCx <- function(data_ROCx, metadata, plot_var, facet_var = NULL, plot_colors = NULL, plotx = TRUE){
  ...
  ggp <- ggplot(data = ROC, aes_string(x = "FPR", y = "TPR", group = plot_var, colour = plot_var)) +
    ...
    if(plotx)
    ggp <- ggp + geom_point(data = X, aes_string(x = "FPR", y = "TPR", group = plot_var, colour = plot_var), size = 10, shape = "X", na.rm=TRUE) 
