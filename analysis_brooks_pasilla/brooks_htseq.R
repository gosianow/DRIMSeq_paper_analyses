######################################################
# BioC 2.14
# Created 13 Nov 2015 

# Get htseq counts

#######################################################

library("DEXSeq")

setwd("/home/Shared/data/seq/brooks_pasilla/")


##############################################################################
# metadata
##############################################################################

metadata <- read.table("3_metadata/metadata.xls", stringsAsFactors=F, sep="\t", header=T) 

metadata


##############################################################################
# htseq counts
##############################################################################


gtf <- "/home/Shared/data/annotation/Drosophila/Ensembl70/gtf/Drosophila_melanogaster.BDGP5.70.gtf"

DEXSeq.gff <- "/home/Shared/data/annotation/Drosophila/Ensembl70/DEXSeq_1.10.8_gff/Drosophila_melanogaster.BDGP5.70.DEXSeq.flattened.rNO.gff"

# python scripts
pkgDir <- system.file(package="DEXSeq")
pythDir <- file.path(pkgDir, "python_scripts")
list.files(pythDir)


####################################
### crerate gff file 
####################################
# disable aggregation with the option “-r no”


system(paste0("python ", pythDir, "/dexseq_prepare_annotation.py --help "))

python.cmd1 <- paste0("python ", pythDir, "/dexseq_prepare_annotation.py -r no ", gtf, " ", DEXSeq.gff)
cat(python.cmd1)

system(python.cmd1)



####################################
### exon counts
####################################

system(paste0("python ", pythDir, "/dexseq_count.py --help "))

counts_out <- "2_counts/htseq/"
dir.create(counts_out, recursive = TRUE)


python.cmd2 <- with(metadata, paste0("python ", pythDir, "/dexseq_count.py -p ", ifelse(LibraryLayout=="PAIRED", "yes", "no"), " -s no -f bam -r pos ", DEXSeq.gff, " 1_reads/sam/", SampleName, "_s.bam ", counts_out, SampleName , ".counts \n"))
cat(python.cmd2)


for(i in 1:length(python.cmd2)){
  # i = 1 
  system(python.cmd2[i])
  
}


####################################
### Merge counts into one file
####################################


counts_list <- lapply(1:nrow(metadata), function(i){
  # i = 1
  
  cts <- read.table(paste0(counts_out, metadata$SampleName[i], ".counts"), header = FALSE, as.is = TRUE)
  colnames(cts) <- c("group_id", metadata$SampleName[i])
  
  return(cts)
  
  })


counts <- Reduce(function(...) merge(..., by = "group_id", all=TRUE, sort = FALSE), counts_list)

tail(counts)
counts <- counts[!grepl(pattern = "_", counts$group_id), ]


write.table(counts, paste0(counts_out, "htseq_counts.txt"), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)








