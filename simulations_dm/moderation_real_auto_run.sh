...
# Important!
# R32loc has a version of DRIMSeq that returns priorn
# devtools::install_github(repo = "markrobinsonuzh/DRIMSeq", ref = "375400477bef101182e56926fec21fd8979a6bc1")
...
R32loc CMD BATCH --no-save --no-restore "--args rwd='$RWD' out_dir_plots='moderation_real_auto_v2' out_dir_res='moderation_real_auto_v2/run' sim_name='' n=${n} nm=${nm} nd=${nd} prop=${prop} disp=${disp} data_name=${data_name} count_method=${count_method} fig_name='all_' out_suffix='${out_suffix}' pdf_width=14 pdf_height=7 strip_text_size=16 text_size=16" $RCODE/moderation_real_auto_plots_run.R $ROUT/moderation_real_auto_plots_run_n${n}.Rout
tail -v $ROUT/moderation_real_auto_plots_run_n${n}.Rout
