######################################################
## ----- dispersion_plots_run
## <<dispersion_plots_run.R>>
...
out_dir_res <- "dispersion/run/"
out_dir_plots <- "dispersion/"
...
ggp <- ggplot(data = error, aes(y = error, x = dispersion, fill = method)) + 
  geom_boxplot(outlier.size = 0.4, outlier.colour = "grey") +
  theme_bw() +
  ylab("Absolute error") +
...
ggp <- ggplot(data = fp, aes(y = fp, x = dispersion)) + 
  geom_boxplot(outlier.size = 1, fill = "grey") +
