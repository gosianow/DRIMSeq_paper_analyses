...
# Do the sqtl analysis for BLOCKS/SNP randomly assigned to genes. 
# This is a validation method. We want to see how many significant sQTLs will be found. 
# If many, then the method works badly.
...
out_dir <- "sqtlseeker_2_1_analysis_random_blocks/"
...
if(!file.exists(genotypes_path)){
  ...
  snps_files <- list.files(path = paste0(data_dir, "/genotypes/random_blocks"), pattern = "snps_CEU", full.names = TRUE, include.dirs = FALSE)
  
  snps_files <- snps_files[grepl("chr",snps_files) & !grepl("sort",snps_files)]
  
  x <- gsub("^.*chr","", snps_files)
  xx <- gsub("_random_blocks.tsv$", "", x)
