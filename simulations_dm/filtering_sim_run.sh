...
do
  ...
  do
    ...
    echo "n${n}_nm${nm}_${prop}_${run}"
      ...
      R32 CMD BATCH --no-save --no-restore "--args rwd='$RWD' simulation_script='$RCODE/dm_simulate.R' workers=${workers} sim_name='test_' run='run${run}' m=100 n=${n} nm=${nm} nd=0 param_pi_path='$DMPARAMS/kim_kallisto/${prop}.txt' param_gamma_path='$DMPARAMS/kim_kallisto/${disp}.txt' max_features=c(Inf,18,8)" $RCODE/filtering_sim_run.R $ROUT/filtering_sim_run_n${n}_nm${nm}_${prop}.Rout
...
R32 CMD BATCH --no-save --no-restore "--args rwd='$RWD' sim_name='' n=${n} nm=${nm} nd=0 prop='${prop}' disp='${disp}'" $RCODE/filtering_sim_plots_run.R $ROUT/filtering_sim_plots_run.Rout
