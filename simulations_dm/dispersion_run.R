...
fp[[1]] <- data.frame(fp = mean(res$pvalue < 0.05, na.rm = TRUE), dispersion = "common", method = "CR")

d <- dmFit(d, dispersion = "genewise_dispersion", BPPARAM = BPPARAM)
d <- dmTest(d, BPPARAM = BPPARAM)
res <- results(d)

fp[[2]] <- data.frame(fp = mean(res$pvalue < 0.05, na.rm = TRUE), dispersion = "genewise", method = "CR")
...
fp[[3]] <- data.frame(fp = mean(res$pvalue < 0.05, na.rm = TRUE), dispersion = "moderated", method = "CR")
